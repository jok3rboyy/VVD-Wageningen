...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/genereerd/data/kandidaten-lijst.json") // Controleer of dit pad correct is
      ...
      kandidaten.forEach((kandidaat) => {
        ...
        const kandidaatHtml = `
                    <div class="team-member">
                        <a href="${
                          kandidaat.path
                        }" class="team-member-link" aria-label="Lees meer over ${
          kandidaat.naam
        }">
                            <img
                                src="${
                                  kandidaat.foto ||
                                  "/images/default-kandidaat.png"
                                }" 
                                alt="" 
                                class="team-photo"
                                loading="lazy"
                            ...
                            <div class="team-info">
                                <h3>${
                                  kandidaat.naam || "Onbekende kandidaat"
                                }</h3>
                                ${
                                  kandidaat.rol
                                    ? `<p class="team-role">${kandidaat.rol}</p>`
                                    : ""
                                }
                                ${
                                  kandidaat.bio_kort
                                    ? `<p class="team-bio">${kandidaat.bio_kort}</p>`
                                    : ""
                                }
                            </div>
                        </a>
    ...
    .catch((error) => {
      ...
      if (kandidatenContainer) {
        kandidatenContainer.innerHTML =
          '<p class="foutmelding">Kon de lijst met teamleden niet laden. Probeer het later opnieuw.</p>';
      }
    });
